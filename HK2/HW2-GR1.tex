...
where $T = 1$ year and $X_t, Y_t$ are the respective underlying spot prices of $X, Y$ at any time $t$.
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\begin{solution}
			...
			Here are the table and the plot of our instant $P\&L$ against percent changes $x, y$ in underlying stock prices (we used the values of question b) for the volatilities, the correlation, the maturity and the risk-free rate).
			...
			\begin{enumerate}
				...
				\item[$\bullet$] Delta-hedged portolio setup cost: $19192049.204$
			\end{enumerate}
		\[
		\linethickness{1pt}
